...
class UsersPage extends Component {
  ...
  Columns = [
    ...
    },
    {
      ...
      key: "userType",
      render: (userType) => (
        <Tag color={userType === "Admin" ? "blue" : "volcano"}>{userType}</Tag>
      ),
      sorter: {
  ...
  componentDidMount() {
    getUsers().then((res) => {
      if (res.data.result) {
        this.setState({
          ListOfUsers: res.data.data,
          ListOfUsersForTable: this.createDataModelForDataTabel(res.data.data),
        });
      }
    });
  ...
  handleEditUser = (userData) => {
    ...
    const userInfo = {
      ..._(this.state.ListOfUsers)
        .filter((c) => c._id === userData._id)
        .first(),
    };
  ...
  handleUserPermissionsChange(checkedValues, permissionName) {
    ...
    access.map((item, index) => {
      if (checkedValues.length === 0) {
        ...
        const existItemInSelectedValues = _(checkedValues)
          .filter((c) => c === item.key)
          .first();
    ...
    editUserInfo(userData)
      .then((response) => {
        if (response.data.result) {
          ...
          const index = _(users).findIndex(
            (c) => c._id === this.state.currentRow._id
          );
          ...
          this.setState({
            ListOfUsers: users,
            ListOfUsersForTable: this.createDataModelForDataTabel(users),
            currentRow: {},
          });
          this.editToggle();
        } else {
  ...
  handleSubmitDeleteUserInfo = () => {
    deleteUserInfo(this.state.currentRow._id)
      .then((response) => {
        if (response.data.result) {
        ...
        if (data.result) {
          getUsers().then((res) => {
            if (res.data.result) {
              this.setState({
                ListOfUsers: res.data.data,
                ListOfUsersForTable: this.createDataModelForDataTabel(
                  res.data.data
                ),
              });
            }
          });
          return toast.success("New users has been added successfully");
        } else {
      ...
      <React.Fragment>
                        ...
                        <Col md="3">
                          <FormGroup>
                            <input
                              ...
                              name="SearchGrid"
                              onChange={(e) =>
                                this.handleSearchGrid(e.target.value)
                              }
                            />
                          </FormGroup>
                        </Col>
                        <Col className="text-right">
                          <FormGroup>
                            <Button
                              color="secondary"
                              type="button"
                              onClick={this.handleImportNewUsersFromBCTS}
                            >
                              <User size={21} color="#FFF" /> Get New Users From
                              BCTS
                            </Button>
                          </FormGroup>
                        </Col>
          ...
          <ModalBody>
            <Row>
              {this.state.currentRow &&
                this.state.currentRow.permissions &&
                this.state.currentRow.permissions.map((permission) => {
                  let access = permission.access.map((item) => {
                    return {
                      label: item.key.split("-").join(" "),
                      value: item.key,
                    };
                  });
                  let defaultValue = permission.access
                      ...
                      <Row>
                        ...
                        >
                          ...
                          <Switch
                            ...
                            unCheckedChildren={
                              !permission.isGranted ? "Not Granted" : ""
                            }
                            onChange={(value) =>
                              this.handleUserPermissionGrantedChange(
                                value,
                                permission.name
                              )
                            }
                        ...
                        <Col md="12" className="ml-1">
                          <Checkbox.Group
                            ...
                            onChange={(checkedValues) =>
                              this.handleUserPermissionsChange(
                                checkedValues,
                                permission.name
                              )
                            }
              ...
              <Col md="12" className="mt-1">
                ...
                {this.UserStatus && (
                  <Radio.Group
                    ...
                    value={
                      this.state.currentRow.isActive ? "Active" : "Inactive"
                    }
        ...
        >
          ...
          <ModalBody>
            ...
            {this.state.currentRow.firstName +
              " " +
              this.state.currentRow.lastName +
              "'s info"}{" "}
            ?
          </ModalBody>
          <ModalFooter>
